    ...
    try:
        with Logging.start_span(request.app.title, "resolve", request=request) as root_span:
            ...
            if x_agent_hint is not None and len(x_agent_hint.strip()) > 0:
                ...
                return DataSourceHub(config=request.app.extra['config']).resolve(request=data_source_request, user_context=context, hint=agent_hint)
            return DataSourceHub(config=request.app.extra['config']).resolve(request=data_source_request, user_context=context)
