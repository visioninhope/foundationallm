...
namespace FoundationaLLM.Utility.Upgrade.Vectorization.State
{
    public class StateUpgrade : Upgrade
    {
        public StateUpgrade(BlobStorageService blobStorageService,
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            _logger = _loggerFactory.CreateLogger<StateUpgrade>();

            _defaultValues = new Dictionary<string, object>();
        }

        protected ILogger<StateUpgrade> _logger;
            ...
            foreach (string name in _datasources.Keys)
            {
                ...
                while (agentVersion < TargetInstanceVersion)
                {
                    ...
                    try
                    {
                        ...
                        var upgrader = (StateUpgrade)Activator.CreateInstance(t, new object[] { _blobStorageService, _instanceSettings, _loggerFactory });
