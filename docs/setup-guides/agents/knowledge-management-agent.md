...
| `vectorization` | The `vectorization` object is only required for non-Inline Context Knowledge Management agents. If the `vectorization` object is included, the `indexing_profile_object_id` and `text_embedding_profile_object_id` keys are required. |
