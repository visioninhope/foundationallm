...
namespace FoundationaLLM.Utility.Upgrade.Infrastructure
{
    public class InfrastructureUpgrade : Upgrade
    {
        public InfrastructureUpgrade(BlobStorageService blobStorageService,
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            _logger = _loggerFactory.CreateLogger<InfrastructureUpgrade>();

            _defaultValues = new Dictionary<string, object>();
        }

        protected ILogger<InfrastructureUpgrade> _logger;
        ...
        {
            var fileContent = await _blobStorageService.ReadFileAsync("resource-provider", "FoundationaLLM.Vectorization/vectorization-text-partitioning-profiles.json");
        ...
        public async override Task UpgradeAsync()
        {
            ...
            foreach (string name in _datasources.Keys)
            {
                ...
                while (agentVersion < TargetInstanceVersion)
                {
                    ...
                    try
                    {
                        string type = $"FoundationaLLM.Common.Upgrade.Vectorization.Paritioning.Vectorization_{agentVersion.ToString().Replace(".", "")}_{targetVersion.ToString().Replace(".", "")}";
                        ...
                        var upgrader = (InfrastructureUpgrade)Activator.CreateInstance(t, new object[] { _blobStorageService, _instanceSettings, _loggerFactory });
