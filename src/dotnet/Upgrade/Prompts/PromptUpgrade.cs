...
{
    public class PromptUpgrade : Upgrade
    {
        public PromptUpgrade(BlobStorageService blobStorageService,
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            _logger = _loggerFactory.CreateLogger<PromptUpgrade>();

            _defaultValues = new Dictionary<string, object>();
        }

        protected ILogger<PromptUpgrade> _logger;
        ...
        {
            var fileContent = await _blobStorageService.ReadFileAsync("resource-provider", $"FoundationaLLM.{TypeName}/_prompt-references.json");

            JsonElement referenceStore = JsonSerializer.Deserialize<dynamic>(Encoding.UTF8.GetString(fileContent.ToArray()));

            JsonElement data = referenceStore.GetProperty("PromptReferences");
            ...
            foreach (string name in _datasources.Keys)
            {
                ...
                while (agentVersion < TargetInstanceVersion)
                {
                    ...
                    try
                    {
                        ...
                        var upgrader = (PromptUpgrade)Activator.CreateInstance(t, new object[] { _blobStorageService, _instanceSettings, _loggerFactory });
