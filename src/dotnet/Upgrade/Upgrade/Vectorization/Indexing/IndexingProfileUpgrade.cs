...
namespace FoundationaLLM.Utility.Upgrade.Vectorization.Indexing
{
    public class IndexingProfileUpgrade : Upgrade
    {
        public IndexingProfileUpgrade(BlobStorageService blobStorageService,
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            _logger = _loggerFactory.CreateLogger<IndexingProfileUpgrade>();

            _defaultValues = new Dictionary<string, object>();
        }

        protected ILogger<IndexingProfileUpgrade> _logger;
        ...
        {
            var fileContent = await _blobStorageService.ReadFileAsync("resource-provider", "FoundationaLLM.Vectorization/vectorization-content-source-profiles.json");
        ...
        public async override Task SaveAsync()
        {
            ...
            foreach (string agentName in _dataSourceObjects.Keys)
            {
                ...
                if (!fileName.Contains("FoundationaLLM.DataSource"))
                    fileName = $"FoundationaLLM.DataSource/{fileName}";
        ...
        public async override Task UpgradeAsync()
        {
            ...
            foreach (string name in _datasources.Keys)
            {
                ...
                while (agentVersion < TargetInstanceVersion)
                {
                    ...
                    try
                    {
                        string type = $"FoundationaLLM.Common.Upgrade.DataSource.DataSource_{agentVersion.ToString().Replace(".", "")}_{targetVersion.ToString().Replace(".", "")}";
                        ...
                        var upgrader = (IndexingProfileUpgrade)Activator.CreateInstance(t, new object[] { _blobStorageService, _instanceSettings, _loggerFactory });
