...
az appconfig kv import --profile appconfig/kvset --name $env:AZURE_APP_CONFIG_NAME --source file --path ./config/appconfig.json --format json --yes

az storage azcopy blob upload -c agents --account-name $env:AZURE_STORAGE_ACCOUNT_NAME -s "../common/data/agents/*" --recursive --only-show-errors
az storage azcopy blob upload -c data-sources --account-name $env:AZURE_STORAGE_ACCOUNT_NAME -s "../common/data/data-sources/*" --recursive --only-show-errors
az storage azcopy blob upload -c foundationallm-source --account-name $env:AZURE_STORAGE_ACCOUNT_NAME -s "../common/data/foundationallm-source/*" --recursive --only-show-errors
az storage azcopy blob upload -c prompts --account-name $env:AZURE_STORAGE_ACCOUNT_NAME -s "../common/data/prompts/*" --recursive --only-show-errors
az storage azcopy blob upload -c resource-provider --account-name $env:AZURE_STORAGE_ACCOUNT_NAME -s "../common/data/resource-provider/*" --recursive --only-show-errors
