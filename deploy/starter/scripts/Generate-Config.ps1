...
if ($deployAks)
{
    ...
    $gvaluesTemplatePath = $(../../common/scripts/Join-Path-Recursively -pathParts $gvaluesTemplate.Split(","))
    $outputFilePath = $(../../common/scripts/Join-Path-Recursively -pathParts $outputFile.Split(","))
    & ../../common/scripts/Token-Replace.ps1 -inputFile $gvaluesTemplatePath -outputFile $outputFilePath -tokens $tokens
    Pop-Location
}
